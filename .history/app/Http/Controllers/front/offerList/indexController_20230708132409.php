    ...
    public function data(Request $request)
    {
        ...
        if($request->status) {
            if($request->status == 'Aktif')
            {
                ...
                $table2->where('status', 1);
                $table3->where('status', 1);
                $table4->where('status', 1);
                $table5->where('status', 1);
            }
            if($request->status == 'Pasif')
            {
                $table->where('status', 0);
                $table2->where('status', 0);
                $table3->where('status', 0);
                $table4->where('status', 0);
                $table5->where('status', 0);
            }
        }
