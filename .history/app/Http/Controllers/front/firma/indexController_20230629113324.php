...
class indexController extends Controller
{
    ...
    public function recordData( Request $request)
    {
        ...
        $a = $table->count();
        $b = $table->where('status','Aktif')->count();
        $filteredPasif = $table->where('status', 'Pasif');
        $renderedData['pasifTotal'] = $filteredPasif->count();

        $filteredAktif = clone $filteredPasif;
        $renderedData['aktifTotal'] = $filteredAktif->where('status', 'Aktif')->count();
